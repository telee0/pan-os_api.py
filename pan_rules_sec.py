...
def pan_rules_sec(dg=None, seq=0):
    key = 'N_RULES_SEC'
    ...
    print("\nPolicies > Security ({0})".format(n), end=" ", flush=True)
    ...
    #
    dst_zone = cf['SEC_DST_ZONE']
    src_zone = cf['SEC_SRC_ZONE']
    service = cf['SEC_SERVICE']
    action = cf['SEC_ACTION']
    ...
    for i in range(256):
        for j in range(256):
            ...
            rule_name = (cf['SEC_NAME'] + suf) % rules
            src = cf['SEC_SOURCE'] % (i, j)
            dst = cf['SEC_DESTINATION'] % (i, j)
            ...
            element = f"""
                ...
                </entry>"""  # .format(rule_name, dst_zone, src_zone, src, dst, service, action)
    ...
    data['dump'].append("</rules></security>")
